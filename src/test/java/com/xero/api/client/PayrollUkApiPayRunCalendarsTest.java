...
public class PayrollUkApiPayRunCalendarsTest {
    ...
    public void getPayRunCalendarsTest() throws IOException {
        ...
        assertThat(response.getPayRunCalendars().get(0).getPayrollCalendarID(),is(equalTo(UUID.fromString("216d80e6-af55-47b1-b718-9457c3f5d2fe"))));
        assertThat(response.getPayRunCalendars().get(0).getCalendarType(), is(equalTo(com.xero.models.payrolluk.PayRunCalendar.CalendarTypeEnum.WEEKLY)));
        assertThat(response.getPayRunCalendars().get(0).getName() , is(equalTo("Weekly ")));
        assertThat(response.getPayRunCalendars().get(0).getPeriodStartDate(),  is(equalTo(LocalDate.of(2020, 02, 10))));
        assertThat(response.getPayRunCalendars().get(0).getPeriodEndDate(),  is(equalTo(LocalDate.of(2020, 02, 16))));
        assertThat(response.getPayRunCalendars().get(0).getPaymentDate(),  is(equalTo(LocalDate.of(2020, 02, 17))));
        assertThat(response.getPayRunCalendars().get(0).getUpdatedDateUTC(), is(equalTo(LocalDateTime.of(2020, 02, 13, 16, 53 ,54))));  
    ...
    public void getPayRunCalendarTest() throws IOException {
        ...
        PayRunCalendarObject response = payrollUkApi.getPayRunCalendar(accessToken, xeroTenantId, payRunCalendarID);
        
        assertThat(response.getPayRunCalendar().getPayrollCalendarID(),is(equalTo(UUID.fromString("216d80e6-af55-47b1-b718-9457c3f5d2fe"))));
        ...
        assertThat(response.getPayRunCalendar().getPeriodStartDate(),  is(equalTo(LocalDate.of(2020, 02, 10))));
        assertThat(response.getPayRunCalendar().getPeriodEndDate(),  is(equalTo(LocalDate.of(2020, 02, 16))));
        assertThat(response.getPayRunCalendar().getPaymentDate(),  is(equalTo(LocalDate.of(2020, 02, 17))));
        assertThat(response.getPayRunCalendar().getUpdatedDateUTC(), is(equalTo(LocalDateTime.of(2020, 02, 13, 16, 53 ,54))));  
    ...
    public void createPayRunCalendarTest() throws IOException {
        ...
        assertThat(response.getPayRunCalendar().getUpdatedDateUTC(), is(equalTo(LocalDateTime.of(2020, 04, 13, 17, 01 , 06).plusNanos(494433100))));  
